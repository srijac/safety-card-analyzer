...
- Both labs judged these models **low-concern on CBRN, cybersecurity, and autonomy**.
  The clear divergence is **persuasion** — GPT-4o is **Medium (marginal)**; the Claude
...
![Who discloses what — topic by card](output/disclosure_matrix.png)
...
python extract_risk_ratings.py && python visualize.py
```
...
| `visualize.py`, `extract_risk_ratings.py` | figure generation |
| `discoveries/` | per-card section inventories |
| `docs/figures/`, `output/` | interactive + hi-res figures |
